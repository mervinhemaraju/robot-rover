...
**Candidate hardware change (evaluated, not scheduled): ESP32 in place of the Arduino Uno R4 Minima.**
A 30-pin ESP32 (CP2102, USB-C) is on hand and is a viable co-processor replacement. Not committed to; revisit before battery telemetry is wired.

* Why it is attractive: roughly half the footprint (frees top-deck space), 240MHz dual-core vs 48MHz, hardware PCNT pulse counters (4 encoders in Phase 4 with zero CPU cost), and LEDC PWM that can run at 20kHz so the motors stop whining.
* Porting cost is low: `arduino/rover_motors/` uses only `analogWrite`/`digitalWrite`, both supported by the ESP32 Arduino core 3.x. Mostly a `config.h` pin remap plus a full F/B/L/R re-verify.
* **3.3V logic and NOT 5V tolerant on any pin.** Driving the L298N is fine (logic HIGH threshold 2.3V) and the incoming DRV8871s are happier still at 3.3V. But nothing 5V may feed back into a pin: the HC-SR04 ECHO line needs a divider in Phase 5, where the R4 did not care.
* **Battery telemetry divider must be resized.** The 10k/10k plan (8.4V -> 4.2V) is sized for a 5V analog pin; 4.2V into an ESP32 pin damages it. Approx 10k/3.3k (8.4V -> ~2.1V) instead. Must use an **ADC1** pin (GPIO 32-39): ADC2 is unusable while WiFi is active.
* Pin constraints: GPIO 6-11 do not exist on the 30-pin board (internal flash). Avoid GPIO 0, 2, 12, 15 for motor direction outputs (strapping pins; GPIO 12 must be LOW at boot). Safe set for the six motor lines: 25, 26, 27, 14, 32, 33.
* Enumerates as `/dev/ttyUSB0` (cp210x), not `/dev/ttyACM0`. Pi side already reads `SERIAL_PORT` from the environment, so this is a unit-file change, not a code change.
* More brownout-sensitive than the R4 under motor load. Feed it from the buck or Pi USB, never from the L298N's onboard 5V regulator.
* **Timing:** if it happens, do it *before* battery telemetry, and ideally in the same teardown as the DRV8871 swap (both rewire the motor logic lines). Doing it after Phase 4 means redoing the divider and four encoder inputs. Keep the R4 Minima as a known-good fallback either way.
