...
3. **Wire the Flutter app to the Pi** at `192.168.0.4` (control 8765, video 8080). Next up. App must STREAM the held command (~every 100 ms), not send once, or the watchdog/failsafe will stop the rover. Steering: positive `steer` = turn right (tunable via `STEER_GAIN`).
