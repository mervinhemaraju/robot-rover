...
**Hardware remount in progress (2026-08-16):** Rover fully unmounted from the chassis — every component (motors, drivers, Pi, Arduino, buck converters, battery holder, camera) currently off the car. Plan before resuming any Phase 3/4/5 software work:

1. **Solder the DRV8871 boards.** Header pins/screw terminals need soldering before they can be wired in (solder wire on hand, iron on hand — see inventory). Blocks everything below.
2. **Full remount**, in order: ground deck (battery holder → buck converter #1 for Pi → buck converter #2 for servos → 2x DRV8871 → Pi) → motors/wheels + encoder discs at all 4 corners → 4x photo interrupter sensors bracketed at each wheel → top deck (Arduino + breadboard) → DRV8871 logic + motor lead wiring → photo interrupter sensor wiring (mount only, no PID code yet — that's Phase 4) → pan-tilt bracket + camera on the new 30cm CSI ribbon, servo signal to D10/D11, servo power from buck converter #2 with common ground to the Arduino.
3. **Motor driver swap:** 2x L298N being replaced by 2x DRV8871 (qty 3 owned, 2 in use + 1 spare) — near-zero voltage drop vs. the L298N's measured 1.73V loss. `arduino/rover_motors/config.h` pin mapping needs updating for the swap (DRV8871 uses 2 logic pins per side vs. L298N's 3), plus new pin assignments for the 4 photo interrupter sensors.
4. **Buck converter #1 must be re-verified at 5V with the DM850 before it's ever reconnected to the Pi** — disassembly means the prior calibration can't be assumed to still hold.
5. Direction wiring (left channel, back-right motor) was previously fixed at the L298N terminals during Phase 2 — those fixes do not carry over to the new DRV8871 wiring and must be re-verified from scratch via Serial Monitor once remounted.

Scope note: mounting the Phase 4 (photo interrupters) and Phase 5 (pan-tilt/servos) hardware now is a deliberate hardware-prep decision ahead of the phase gate — no PID or servo-control software will be written until Phase 3 remaining items (camera screen wiring, battery telemetry, BT fallback) are actually complete.

**Co-processor decision (settled 2026-07-25): the Arduino Uno R4 Minima stays.** The ESP32 was evaluated as a replacement and rejected; it is kept as a bench-prototyping board only. Do not re-propose the swap. Battery telemetry therefore uses the original 10k/10k divider (8.4V -> 4.2V) into a 5V-tolerant R4 analog pin.
